...
def validate_on_data(
    ...
    with torch.no_grad():
        ...
        total_num_seqs = 0
        for valid_batch, valid_batch_ground in zip(valid_iter, valid_iter_ground):
            batch = Batch(
                is_train=False,
        ...
        if do_translation:
            ...
            if i == 1:
                # TXT Metrics
                FID = fid(references=all_encoder_outputs_ground, hypotheses=all_encoder_outputs)
                MPJPE = mpjpe(references=all_sign_ground, hypotheses=all_sign)
                MPVPE = 0 #mpvpe(references=all_sign_ground, hypotheses=all_sign)
                MPJAE = mpjae(references=all_sign_ground, hypotheses=all_sign)
        ...
        if do_translation:
            valid_scores["mpjpe"] = MPJPE
            valid_scores["mpvpe"] = MPVPE
            valid_scores["mpjae"] = MPJAE
            valid_scores['fid'] = FID
...
def test(
            ...
            for ta in translation_beam_alphas:
                ...
                if (
                    ...
                    logger.info(
                        ...
                        dev_best_translation_result["valid_scores"]["fid"],
                        dev_best_translation_result["valid_scores"]["mpjpe"],
                        dev_best_translation_result["valid_scores"]["mpvpe"],
                        dev_best_translation_result["valid_scores"]["mpjae"],
                    )
                    logger.info("-" * 60)
    ...
    logger.info(
        ...
        dev_best_translation_result["valid_scores"]["fid"] if do_translation else -1,
        dev_best_translation_result["valid_scores"]["mpjpe"] if do_translation else -1,
        dev_best_translation_result["valid_scores"]["mpvpe"] if do_translation else -1,
        dev_best_translation_result["valid_scores"]["mpjae"] if do_translation else -1,
    )
    logger.info("*" * 60)
    ...
    logger.info(
        ...
        test_best_result["valid_scores"]["fid"] if do_translation else -1,
        test_best_result["valid_scores"]["mpjpe"] if do_translation else -1,
        test_best_result["valid_scores"]["mpvpe"] if do_translation else -1,
        test_best_result["valid_scores"]["mpjae"] if do_translation else -1,
    )
    logger.info("*" * 60)
